...
const TablePaginationLegacy = ({ currentPage = 0, links = null, totalPages = 0 }) => {
  ...
  const displayPrevSelfNextButtons = (
    prevPage = 0,
    selfPage = 0,
    nextPage = 0
  ) => {
  ...
  if (totalPages === 0) {
    ...
    if (currentPage === 2) {
      ...
      const linkTwo = currentPage === 1 ? nextLink : prevLink;
      pager = (
        <Fragment>
          <PaginationButton pageLink={firstLink} pageNumber={1} />
  ...
  return (
    <div aria-label="pagination" style={{ marginTop: '10px' }}>
      {pager}
    </div>
  );
};

TablePaginationLegacy.propTypes = propTypes;
export default TablePaginationLegacy;
